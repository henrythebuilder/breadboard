defmodule SwitchServerTest do
    ...
    :ok =
      GenServer.call(
        ...
        {:set_interrupts,
         [
           opts: [],
           interrupts_receiver: &InterruptsTest.interrupt_service_routine/1,
           trigger: :both
         ]}
      )
  ...
  test "Manage pin interrupts (enable/disable) by message" do
    ...
    :ok =
      GenServer.call(
        pid0,
        {:set_interrupts, [opts: [], interrupts_receiver: self(), trigger: :both]}
      )

    # after calling set_interrupts, the calling process will receive an initial message with the state of the pin
    assert_receive {:irq_service_call,
                    %Breadboard.IRQInfo{new_value: 0, pin_label: :gpio1, pin_number: 1}}

    GenServer.call(pid1, :turn_on)

    assert_receive {:irq_service_call,
                    %Breadboard.IRQInfo{new_value: 1, pin_label: :gpio1, pin_number: 1}}
